...
Функция ЗапускВКоманднойСтроке()
	...
	Попытка
		...
		ЗначенияПараметров = Аргументы.ЗначенияПараметров;

		Для Каждого Параметр Из ЗначенияПараметров Цикл
			Лог.Отладка("%1 = %2", Параметр.Ключ, Параметр.Значение);
		КонецЦикла;
		
		ДанныеПодключения = ДанныеПодключения(ЗначенияПараметров);
		
		Если Команда = ВозможныеКоманды().Декомпилировать Тогда
			Декомпилировать(ЗначенияПараметров["ПутьВходящихДанных"], ЗначенияПараметров["ВыходнойКаталог"]);
		ИначеЕсли Команда = ВозможныеКоманды().Помощь Тогда
			ВывестиСправку();
		ИначеЕсли Команда = ВозможныеКоманды().ОбработатьИзмененияИзГит Тогда
			ОбработатьИзмененияИзГит(ЗначенияПараметров["ВыходнойКаталог"], ЗначенияПараметров["--remove-orig-bin-files"],
				ДанныеПодключения);
		ИначеЕсли Команда = ВозможныеКоманды().Компилировать Тогда
			Компилировать(
				ЗначенияПараметров["ПутьВходящихДанных"],
				ЗначенияПараметров["ВыходнойКаталог"],
				ЗначенияПараметров["--recursive"]
		);
	ИначеЕсли Команда = ВозможныеКоманды().Установить Тогда
		УстановитьВКаталог(ТекущийКаталог(), ДанныеПодключения);
...
Процедура УстановитьВКаталог(Знач Каталог, Знач ДанныеПодключения = Неопределено) Экспорт
	...
	ИМЯ_ФАЙЛА_ХУКА = "pre-commit";
	ИМЯ_ФАЙЛА_СКРИПТА = "v8files-extractor.os";
	
	КопироватьВКаталог(КаталогПрекоммита, КаталогХуков, ИМЯ_ФАЙЛА_ХУКА);
	ДополнитьФайлХукаДаннымиПодключения(ОбъединитьПути(КаталогХуков, ИМЯ_ФАЙЛА_ХУКА), ИМЯ_ФАЙЛА_СКРИПТА, 
		ДанныеПодключения);

	КопироватьВКаталог(КаталогПрекоммита, КаталогХуков, "v8Reader");
	КопироватьВКаталог(КаталогПрекоммита, КаталогХуков, ИМЯ_ФАЙЛА_СКРИПТА);
...
Процедура ДополнитьФайлХукаДаннымиПодключения(Знач ПутьФайлаХуков, Знач ИмяФайлаСкрипта, Знач ДанныеПодключения)
	Если Не ЗначениеЗаполнено(ДанныеПодключения) Тогда
		Возврат;
	КонецЕсли;

	ФайлХука = Новый Файл(ПутьФайлаХуков);
	ПутьФайлаХуков = ФайлХука.ПолноеИмя;
	Ожидаем.Что(ФайлХука.Существует(), "Ожидали, что файл существует, а это  не так " + ПутьФайлаХуков).
		ЭтоИстина();
	
	ТекстФайла = ПрочитатьФайл(ПутьФайлаХуков);
	
	ШаблонВставки = "oscript -encoding=utf-8 .git/hooks/v8files-extractor.os --git-precommit src %1 %2 %3 %4 %5 %6";
	НовыйТекстФайла = "";
	Для Счетчик = 1 По СтрЧислоСтрок(ТекстФайла) Цикл
		Строка = СтрПолучитьСтроку(ТекстФайла, Счетчик);

		Если СтрНайти(Строка, ИмяФайлаСкрипта) <> 0
		И СтрНайти(Строка, ВозможныеКоманды().ОбработатьИзмененияИзГит) <> 0 Тогда

			Строка = СтрШаблон(ШаблонВставки, 
				"--ibconnection", ДанныеПодключения.ПутьБазы,
				"--db-user", ДанныеПодключения.Пользователь,
				"--db-pwd", ДанныеПодключения.Пароль);
			
		КонецЕсли;

		Если ПустаяСтрока(НовыйТекстФайла) Тогда
			НовыйТекстФайла = Строка;
		Иначе
			НовыйТекстФайла = НовыйТекстФайла + Символы.ПС + Строка;
		КонецЕсли;
	КонецЦикла;
	
	СохранитьФайл(ПутьФайлаХуков, НовыйТекстФайла);
...
Процедура СохранитьФайл(Знач Путь, Знач Текст)
...
Процедура ОбработатьИзмененияИзГит(Знач ВыходнойКаталог, Знач УдалятьФайлыИзИндексаГит, Знач ДанныеПодключения) Экспорт
