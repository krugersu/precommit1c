...
Функция ЗапускВКоманднойСтроке()
	...
	Попытка
		...
		Парсер.ДобавитьПараметрФлаг("--debug", , Истина);
		...
		Если Аргументы.ЗначенияПараметров["--debug"] Тогда
			Лог.УстановитьУровень(УровниЛога.Отладка);
		КонецЕсли;		
...
Процедура Инициализация()
	...
	//Лог.УстановитьУровень(УровниЛога.Отладка);
	ДобавитьЛогФайлОтладки(Лог);
	ЛогРаннера = Логирование.ПолучитьЛог("oscript.lib.v8runner") ;
	ЛогРаннера.УстановитьУровень(Лог.Уровень());
	ДобавитьЛогФайлОтладки(	ЛогРаннера );

КонецПроцедуры

Процедура ДобавитьЛогФайлОтладки(ОбъектЛога)
	 ВыводПоУмолчанию = Новый ВыводЛогаВКонсоль();
	 //ОбъектЛога.ДобавитьСпособВывода(ВыводПоУмолчанию);

	ВыводЛогаВФайл = Новый ВыводЛогаВФайл;
	// ФайлИсточника = Новый Файл(ТекущийСценарий().Источник);
	// ПутьЛога = ОбъединитьПути(ФайлИсточника.Путь, ФайлИсточника.ИмяБезРасширения + ".log" );
	ПутьЛога = ПолучитьИмяВременногоФайла("log");
	// ОбъектЛога.Отладка("Перед ВыводЛогаВФайл.ОткрытьФайл(ПутьЛога) "+ПутьЛога);
	ВыводЛогаВФайл.ОткрытьФайл(ПутьЛога);
	// Сообщить("После ВыводЛогаВФайл.ОткрытьФайл(ПутьЛога) "+ПутьЛога);
	// ОбъектЛога.Отладка("После ВыводЛогаВФайл.ОткрытьФайл(ПутьЛога) "+ПутьЛога);
	ОбъектЛога.ДобавитьСпособВывода(ВыводЛогаВФайл);
	// ОбъектЛога.Отладка("Лог.ДобавитьСпособВывода(ВыводЛогаВФайл) "+ПутьЛога);

	ОбъектЛога.ДобавитьСпособВывода(ЭтотОбъект);
КонецПроцедуры

//{ Методы аппендера

Процедура Вывести(Знач Сообщение) Экспорт
	// мСообщенияЛога.Добавить(""+ТекущаяДата+": " + Сообщение);
...
Процедура ЗапуститьРаспаковку(Знач Файл, Знач ПапкаИсходников)
	...
	Ожидаем.Что(Новый Файл(ПутьV8Reader).Существует(), "Ожидаем, что файл "+ПутьV8Reader+" существует").ЭтоИстина();
...
Процедура ВывестиСправку()
	...
	Сообщить("	--decompile inputPath outputPath [--debug]");
	Сообщить("		Разбор файлов на исходники");
	Сообщить("		inputPath   Путь к файлу, который распаковывается");
	Сообщить("		outputPath   Путь к каталогу, куда распаковывать");
	
	Сообщить("	--help");
	Сообщить("		Показ этого экрана");
	Сообщить("	--git-precommit [outputPath] [--debug]");
	...
	Сообщить("		outputPath  Путь к каталогу, куда распаковывать. Если не задан, используется каталог .\src");
	Сообщить("	--compile inputPath outputPath [--type TYPE] [--debug]");
	...
	Сообщить(" ");
	Сообщить("  Ключ --debug включает режим формирования полных логов, выводимых в консоль");
КонецПроцедуры
