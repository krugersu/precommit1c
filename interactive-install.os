...
Функция СоздатьСсылку(_куда, _откуда)
   ...
   _создательСсылки = создаватель + " " + ВКавычки(_куда) + " " + ВКавычки(_адресОткуда);
   
    Сообщить(_создательСсылки);
    
    ЗапуститьПриложение(_создательСсылки,,Истина);
    
    //mklink ведет себя странно в режиме процесса TODO
...
|   v8files-extractor - скрипт распаковки " + СоздатьСсылку(
  ОбъединитьПути(Ответ,".git","hooks","v8files-extractor"),"v8files-extractor"  
) + " 
...
КопироватьФайл(v8unpackАдрес, КаталогПрограммы());
