...
Функция ПолучитьСписокТестов(Знач Контекст) Экспорт
	...
	ВсеТесты.Добавить("Тест_ДолженПроверитьЧтоПрекоммит1СУстановленВХукиИУказаноПодключениеКБазе");
...
Процедура Тест_ДолженПроверитьЧтоПрекоммит1СУстановленВХукиИУказаноПодключениеКБазе() Экспорт
	...
	ДанныеПодключения = Новый Структура("ПутьБазы, Пользователь, Пароль", "/Fw:\123", "login", "pass");

	Исполнитель.УстановитьВКаталог(КаталогРепо, ДанныеПодключения);
	...
	ПутьФайлаХука = ОбъединитьПути(КаталогHooks.ПолноеИмя, "pre-commit");
	Файл_pre_commit_конечный = Новый Файл(ПутьФайлаХука);
	Лог.Отладка("Файл_pre_commit_конечный.ПолноеИмя " + Файл_pre_commit_конечный);
	Ожидаем.Что(Файл_pre_commit_конечный.Существует(), "Должен существовать Файл_pre_commit_конечный, а это не так").ЭтоИстина();

	ИМЯ_ФАЙЛА_СКРИПТА = "v8files-extractor.os";

	ТекстФайлаХука = ПрочитатьФайл(ПутьФайлаХука);
	Лог.Информация(ТекстФайлаХука);

	НаборПроверок = Новый Соответствие;
	НаборПроверок.Вставить("Ожидаем, что в файле хука pre-commit есть строка с именем скрипта " + ИМЯ_ФАЙЛА_СКРИПТА, 
		СтрНайти(ТекстФайлаХука, ИМЯ_ФАЙЛА_СКРИПТА) > 0);
	НаборПроверок.Вставить("Ожидаем, что в файле хука pre-commit есть строка с командой --git-precommit", 
		СтрНайти(ТекстФайлаХука, "--git-precommit") > 0);
	НаборПроверок.Вставить("Ожидаем, что в файле хука pre-commit есть строка со строкой подключения к ИБ", 
		СтрНайти(ТекстФайлаХука, "--ibconnection " + ДанныеПодключения.ПутьБазы) > 0);
	НаборПроверок.Вставить("Ожидаем, что в файле хука pre-commit есть строка с именем пользователя", 
		СтрНайти(ТекстФайлаХука, "--db-user " + ДанныеПодключения.Пользователь) > 0);
	НаборПроверок.Вставить("Ожидаем, что в файле хука pre-commit есть строка с паролем пользователя", 
		СтрНайти(ТекстФайлаХука, "--db-pwd " + ДанныеПодключения.Пароль) > 0);

	Для каждого КлючЗначение Из НаборПроверок Цикл
		Ожидаем.Что(КлючЗначение.Значение, КлючЗначение.Ключ).ЭтоИстина();
	КонецЦикла;

	Файл_сценария_исходный = Новый Файл(ОбъединитьПути(КаталогПроекта, ИМЯ_ФАЙЛА_СКРИПТА));
	Файл_сценария_конечный = Новый Файл(ОбъединитьПути(КаталогHooks.ПолноеИмя, Файл_сценария_исходный.Имя));
	Ожидаем.Что(Файл_сценария_конечный.Существует(), "Должен существовать Файл_сценария_конечный, а это не так").ЭтоИстина();
