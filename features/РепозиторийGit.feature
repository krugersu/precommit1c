...
Контекст:
    ...
    И Я выполняю команду "git" c параметрами 'init'
...
Сценарий: Проверка настроек репозитория Git (простой отрицательный)
    ...
    И Я выполняю команду "oscript" c параметрами '.git/hooks/v8files-extractor.os --git-check-config'
...
Сценарий: Проверка настроек репозитория Git (простой положительный)
    Когда Я копирую файл "v8files-extractor.os" из каталога "" проекта в подкаталог ".git/hooks" рабочего каталога
    И Я выполняю команду "git" c параметрами "config --local core.quotepath false"
    И Я выполняю команду "oscript" c параметрами ".git/hooks/v8files-extractor.os --git-check-config"
...
Сценарий: Установка precommit1c в репозиторий
    Когда Я выполняю команду "git" c параметрами "config --local core.quotepath false"
    И Я выполняю команду "oscript" c параметрами "<КаталогПроекта>/v8files-extractor.os --install" 
