...
#### Установка через консольный инсталятор

1. откройте командную строку в каталоге установки `precommit1C` - через `Shift+ПКМ -> Открыть окно команд`

2. запустите команду `oscript interactive-install.os`

3. укажите адрес существующего каталога куда необходимо подключить `precommit1c`, например `D:\\My-Epfs`
